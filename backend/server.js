...
const BARBERSHOP_INFO = {
  ...
  concept:
    "Современный барбершоп в Бишкеке: мужские стрижки, борода, бритье и спокойная запись онлайн.",
...
const SERVICES = [
  ...
  },
  {
    ...
    price: 700,
    duration: 20,
    description: "Быстрая аккуратная стрижка одной машинкой",
  },
  {
    id: 3,
  ...
  },
  {
    ...
    price: 1000,
    duration: 30,
    description: "Коррекция формы бороды и контуров",
  },
  {
    id: 6,
    name: "Опасное бритье",
  ...
  },
  {
    ...
    price: 2200,
    duration: 60,
    description: "Натуральное тонирование седины",
  },
  {
    id: 9,
    name: "Укладка",
    ...
    price: 500,
    duration: 20,
    description: "Финишная укладка с профессиональными средствами",
  },
  {
    id: 10,
    name: "Консультация барбера",
    category: "консультация",
    price: 400,
    duration: 20,
    description: "Подбор стрижки и ухода под ваш стиль",
  },
];
...
app.get("/api/slots", (req, res) => {
  ...
  if (barberId) {
    availableSlots = availableSlots.filter(
      (slot) => slot.barberId === parseInt(barberId),
    );
  }
...
app.post("/api/appointments", (req, res) => {
  ...
  STATISTICS.popularTimes[timeKey] =
    (STATISTICS.popularTimes[timeKey] || 0) + 1;
  STATISTICS.popularServices[service.name] =
    (STATISTICS.popularServices[service.name] || 0) + 1;
...
app.get("/api/appointments", (req, res) => {
  ...
  if (clientPhone) {
    appointments = appointments.filter((a) =>
      a.clientPhone.includes(clientPhone),
    );
  }
...
app.post("/api/chat", (req, res) => {
  ...
  if (
    lowerMessage.includes("какая стрижка") ||
    lowerMessage.includes("подойдет") ||
    lowerMessage.includes("посоветуй")
  ) {
    response = {
      type: "fallback",
      message:
        "Я могу помочь с записью, но для консультации по стилю лучше подойдут наши барберы. Хотите, запишу вас на консультацию к мастеру?",
      action: "offer_consultation",
    };
  }
  // Сценарий 2: Клиент недоволен
  else if (
    lowerMessage.includes("жалоба") ||
    lowerMessage.includes("недоволен") ||
    lowerMessage.includes("плохо") ||
    lowerMessage.includes("ужасно")
  ) {
    response = {
      type: "fallback",
      message:
        "Приношу извинения за неудобства. Я передаю ваш контакт управляющему, чтобы он лично решил этот вопрос.",
      action: "connect_manager",
    };
  }
  // Сценарий 3: Услуга не найдена
  else if (
    lowerMessage.includes("женск") ||
    lowerMessage.includes("девочк") ||
    lowerMessage.includes("маникюр")
  ) {
    response = {
      type: "fallback",
      message:
        "Мы специализируемся на мужских стрижках. Могу предложить услуги для ваших детей или классическую мужскую стрижку.",
  ...
  // Приветствие
    ...
    lowerMessage.includes("добрый")
  ) {
    response = {
      type: "greeting",
      message:
        "Добрый день! Это AI-ассистент Barber Bee Бишкек. Чем могу помочь?",
      suggestions: [
        "Узнать цены в сомах",
  ...
  // Запрос акции
    ...
    lowerMessage.includes("акция") ||
    lowerMessage.includes("скидка") ||
    lowerMessage.includes("бонус")
  ) {
    response = {
      type: "promo",
      message:
        "У нас сейчас действует акция: при записи на камуфляж седины на этой неделе скидка 15%. Также в день рождения дарим скидку 20% на любую услугу.",
      action: "offer_promo",
    };
  }
  // Общий ответ
    ...
    response = {
      type: "general",
      message:
        "Я могу помочь с записью на стрижку, ценами в сомах и выбором мастера. Что вас интересует?",
